...
import { ExerciseEquipment } from '@/config/types';
...
interface AdminEquipmentClientProps {
    initialEquipment: ExerciseEquipment[];
}

export default function AdminEquipmentClient({ initialEquipment }: AdminEquipmentClientProps) {
    const [equipment, setEquipment] = useState<ExerciseEquipment[]>(initialEquipment);
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [editingEquipment, setEditingEquipment] = useState<ExerciseEquipment | null>(null);
    ...
    const openCreateModal = () => {
        ...
        setName('');
        setImageUrl('');
        setDisplayOrder(equipment.length + 1);
        setIsActive(true);
    ...
    const openEditModal = (eq: ExerciseEquipment) => {
        setEditingEquipment(eq);
        setSlug(eq.slug);
        setName(eq.name);
        setImageUrl(eq.imageUrl || '');
        setDisplayOrder(eq.displayOrder || 0);
        setIsActive(eq.isActive ?? true);
        setTranslations({
            pt: eq.translations?.pt || eq.name || '',
            en: eq.translations?.en || '',
            es: eq.translations?.es || ''
        });
        setActiveTab('pt');
        setIsModalOpen(true);
    };
    ...
    const handleSave = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!slug.trim()) {
            toast.error('O slug é obrigatório (ex: barbell, trx_suspension).');
            return;
        }
        ...
        try {
            ...
            const payload: Partial<ExerciseEquipment> = {
            ...
            if (editingEquipment && editingEquipment.id) {
                await taxonomyService.updateEquipmentAdmin(editingEquipment.id, payload, supabase);
                toast.success('Equipamento atualizado com sucesso!');
            } else {
                const created = await taxonomyService.createEquipmentAdmin(payload, supabase);
                payload.id = created.id;
                toast.success('Equipamento criado com sucesso!');
            }

            // Recarrega lista
            const updated = await taxonomyService.getAllEquipmentAdmin(supabase);
            setEquipment(updated);
            setIsModalOpen(false);
        } catch (err: any) {
            console.error('[AdminEquipmentClient] Error saving equipment:', err);
    ...
    const handleDelete = async (id?: string) => {
        if (!id) return;
        if (!confirm('Tem certeza de que deseja remover este equipamento? Isso pode afetar exercícios vinculados a ele.')) {
            return;
        }

        try {
            const supabase = createClient();
            await taxonomyService.deleteEquipmentAdmin(id, supabase);
            toast.success('Equipamento excluído!');
            setEquipment(equipment.filter(e => e.id !== id));
        } catch (err: any) {
            console.error('[AdminEquipmentClient] Error deleting equipment:', err);
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-900 pb-6">
                <div className="space-y-1">
                    ...
                    <p className="text-zinc-500 text-sm">
                        Cadastre novos equipamentos (TRX, Kettlebell, Smith, etc.) ou edite os existentes.
                    </p>
                </div>
                <button
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="flex items-center gap-3">
                                {eq.imageUrl ? (
                                    ...
                                    <div className="w-12 h-12 rounded-xl flex items-center justify-center font-black text-lg bg-zinc-800 text-lime-400 border border-zinc-700">
                                        <Dumbbell size={20} />
                                    </div>
                                )}
                                <div>
                                    <h3 className="font-bold text-base text-white">{eq.name}</h3>
                                    <code className="text-xs text-zinc-400 font-mono">slug: {eq.slug}</code>
                                </div>
                            </div>
                            ...
                            <div className="space-y-1 pt-2 border-t border-zinc-800/50 text-xs">
                                <div className="flex items-center gap-2 text-zinc-400">
                                    <span>🇧🇷</span>
                                    <span>{eq.translations?.pt || eq.name}</span>
                                </div>
                                <div className="flex items-center gap-2 text-zinc-400">
                                    <span>🇺🇸</span>
                                    <span>{eq.translations?.en || <em className="text-zinc-600">Não traduzido</em>}</span>
                                </div>
                                <div className="flex items-center gap-2 text-zinc-400">
                                    <span>🇪🇸</span>
                                    <span>{eq.translations?.es || <em className="text-zinc-600">Não traduzido</em>}</span>
                                </div>
                            </div>
                        </div>
                ...
                <form onSubmit={handleSave} className="space-y-5 p-6 text-white">
                    ...
                    <div className="space-y-1.5">
                        ...
                        <input
                            ...
                            placeholder="/images/equipaments/barbell.png"
                    ...
                    <div className="space-y-3 pt-2">
                        ...
                        <div>
                            <input
                                ...
                                placeholder={`Nome do equipamento em ${SUPPORTED_LOCALES.find(l => l.code === activeTab)?.label}...`}
                                className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-lime-400"
                            />
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 pt-2">
                        ...
                        />
                        <label htmlFor="eqIsActive" className="text-xs font-bold text-zinc-300 cursor-pointer">
