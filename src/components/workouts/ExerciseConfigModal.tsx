...
import { Plus, Trash2, Dumbbell, Activity, HelpCircle, Save } from 'lucide-react';
import { Modal } from '@/components/ui/Modal';
import { ExerciseGroup, WorkoutExercise, PlannedSet, GroupType, SetTechnique } from '@/config/types';
...
export const ExerciseConfigModal: React.FC<ExerciseConfigModalProps> = ({
    ...
    const [group, setGroup] = useState<ExerciseGroup>(DEFAULT_GROUP);
    ...
    useEffect(() => {
        if (isOpen) {
            if (groupData) {
                // Clone group data deep copy to prevent mutating props directly
    ...
    const handleConfirm = (e: React.FormEvent) => {
        e.preventDefault();
        // Check if any exercise hasn't been selected yet
    ...
    return (
        <>
            <Modal
                isOpen={isOpen}
                onClose={onClose}
                title={groupData?.exercises?.[0]?.exerciseName ? (te.has(groupData.exercises[0].exerciseName) ? te(groupData.exercises[0].exerciseName) : groupData.exercises[0].exerciseName) : t('selectExercise')}
                maxWidth="max-w-2xl"
            >
