...
export default function HistoryClient({ userId, initialHistoryList, initialDate, initialWorkoutId }: HistoryClientProps) {
    ...
    useEffect(() => {
        if (!hasOpenedInitial && initialDate && initialWorkoutId && Object.keys(workoutData).length > 0) {
            ...
            if (workoutData[dateKey] && workoutData[dateKey].some(w =>
                String(w.workoutId) === String(initialWorkoutId) ||
                String(w.id) === String(initialWorkoutId)
            )) {
    ...
    const endMonthDate = useMemo(() => new Date(currentDate.getFullYear(), currentDate.getMonth() + 1, 0), [currentDate]);

    useEffect(() => {
        // Se for o mês inicial e já temos os dados, não carregar
        const now = new Date();
        if (currentDate.getMonth() === now.getMonth() && currentDate.getFullYear() === now.getFullYear() && historyList === initialHistoryList) {
            return;
        }
        ...
        fetchHistory();
    }, [userId, currentDate, startMonthDate, endMonthDate, initialHistoryList]);
