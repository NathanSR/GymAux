...
export default function ExerciseLibraryPage() {
    ...
    }, [searchQuery, selectedCategory, exercises, te]);
                ...
                <div className="grid grid-cols-1 gap-4">
                    {filteredExercises.length > 0 ? (
                        filteredExercises.map(exercise => {
                            // Tenta buscar tradução no JSON "Exercises", se não achar, usa o valor original do banco
                            const name = te(`${exercise.name}`);
                            const description = te(`${exercise.description}`);
